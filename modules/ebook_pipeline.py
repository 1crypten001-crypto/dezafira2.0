...
class EbookMacroPipeline:
    ...
    async def _phase_producao(self):
        ...
        from agents.specialists import chapter_writer
        ...
        # 2. Gerar estrutura (sumario) via ChapterWriterAgent
        ...
        self.state.chapters_structure = await chapter_writer.generate_structure(
            book_title=self.state.book_title,
            niche=self.state.niche,
            num_chapters=target,
            promise=promise,
        ...
        for idx, ch in enumerate(self.state.chapters_structure):
            ...
            # Gerar conteudo do capitulo via ChapterWriterAgent
            ch_result_agent = await chapter_writer.write_chapter(
                title=ch_title,
                chapter_number=ch_num,
                total_chapters=len(self.state.chapters_structure),
                niche=self.state.niche,
                book_title=self.state.book_title,
                promise=promise,
                mechanism_name=mechanism.get("name", ""),
            ...
            chapter_resp = ch_result_agent.get("content", "")
        ...
        # 4. Gerar capa via CoverDesignerAgent
        self._update_macro(sid, "active", 85, "Gerando capa...")
        try:
            from agents.specialists import cover_designer
            from modules.image_factory import ImageGeneratorAgent
            cover_design = await cover_designer.design_cover(
                self.state.book_title, self.state.niche, self.state.style_id
            )
            img_agent = ImageGeneratorAgent()
            cover_result = await img_agent.generate_for_ebook(cover_design["prompt"])
            if cover_result and cover_result.get("image_url"):
                self.state.cover_url = cover_result["image_url"]
                from modules.database import update_db_book
                update_db_book(self.state.book_id, cover_url=cover_result["image_url"])
        except Exception as e:
...
{chapters_html}
    ...
    async def _phase_entrega(self):
        from modules.database import (
            update_db_book, update_db_ebook_pipeline_run,
            get_db_book
        )
        ...
        update_db_book(self.state.book_id, status="published")
