...
# ═══════════════════════════════════════════════════════════════════════════════
# DEZAFIRA CLUB — Modelos de Usuário e Auth
# ═════════════════════════════════════════════════════════════════════════════════════════════════════
...
class DistributionSettings(Base):
    """Persistência do agendador automático de distribuição social.
    Linha única (id='default'): sobrevive a restarts do servidor."""
...
class MiniApp(Base):
    ...
    app_type = Column(String(50), default="Interactive PWA")
    logo_url = Column(String(1000), nullable=True)
    banner_url = Column(String(1000), nullable=True)
    pwa_manifest = Column(Text, nullable=True)    # JSON
    pwa_html = Column(Text, nullable=True)        # HTML/CSS/JS gerado pelo agente
...
class MiniAppDripContent(Base):
    ...
    __tablename__ = "miniapp_drip_contents"

    id = Column(Integer, primary_key=True, autoincrement=True)
    miniapp_id = Column(String(50), ForeignKey("miniapps.id"), nullable=False, index=True)
    unlock_day = Column(Integer, nullable=False)
...
try:
    ...
    with engine.connect() as conn:
        ...
        _migrate_add_column(conn, "ALTER TABLE users ADD COLUMN IF NOT EXISTS asaas_customer_id VARCHAR(100);", "users.asaas_customer_id")
        _migrate_add_column(conn, "ALTER TABLE users ADD COLUMN IF NOT EXISTS asaas_subscription_id VARCHAR(100);", "users.asaas_subscription_id")
        _migrate_add_column(conn, "ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_expires_at TIMESTAMP;", "users.subscription_expires_at")
...
try:
    _db_setup = SessionLocal()
    _admin_email = "admin@dezafira.com"
    ...
    if not _existing_user:
        ...
        if _existing_user.plan != "pro" or _existing_user.role != "admin":
            ...
            _existing_user.role = "admin"
            _db_setup.commit()
            print(f"[Database] Usuario de teste '{_admin_email}' atualizado para PRO e Admin.")
except Exception as _setup_err:
    print(f"[Database] Aviso ao configurar usuario de teste administrador: {_setup_err}")
finally:
    _db_setup.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
# CRUD — FABRICA DE EBOOKS (PIPELINE)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# MINIAPP FACTORY — CRUD Functions
# ═══════════════════════════════════════════════════════════════════════════════
...
def update_db_miniapp(app_id: str, **kwargs) -> bool:
    """Atualiza campos de um MiniApp."""
    db = SessionLocal()
    try:
...
def get_db_miniapp(app_id: str) -> dict:
    """Retorna detalhes completos de um MiniApp incluindo drip contents."""
    db = SessionLocal()
    try:
        app = db.query(MiniApp).filter(MiniApp.id == app_id).first()
        if not app:
            return {}
        drips = db.query(MiniAppDripContent).filter(
            MiniAppDripContent.miniapp_id == app_id
        ).order_by(MiniAppDripContent.unlock_day).all()
        return {
            "id": app.id, "app_name": app.app_name, "niche": app.niche,
...
def get_db_miniapps(limit: int = 50) -> list:
    """Lista todos os MiniApps ordenados por criacao (mais recentes primeiro)."""
    db = SessionLocal()
    try:
        apps = db.query(MiniApp).order_by(MiniApp.created_at.desc()).limit(limit).all()
        return [{
...
def delete_db_miniapp(app_id: str) -> bool:
    """Deleta um MiniApp e seus drip contents."""
    db = SessionLocal()
    try:
        db.query(MiniAppDripContent).filter(MiniAppDripContent.miniapp_id == app_id).delete()
        app = db.query(MiniApp).filter(MiniApp.id == app_id).first()
        if app:
            db.delete(app)
            db.commit()
...
def create_db_miniapp_drip(miniapp_id: str, unlock_day: int, title: str,
                           content_type: str, payload: str) -> dict:
    """Cria um item de drip content para um MiniApp."""
    db = SessionLocal()
    try:
        drip = MiniAppDripContent(
            miniapp_id=miniapp_id, unlock_day=unlock_day, title=title,
            content_type=content_type, payload=payload,
        )
        db.add(drip)
        db.commit()
        return {"id": drip.id, "miniapp_id": drip.miniapp_id, "unlock_day": drip.unlock_day}
    finally:
        db.close()
...
def get_db_distribution_settings() -> dict:
    """Retorna o estado persistido do agendador de distribuição social.

    Returns:
        {"enabled": bool, "interval_hours": int} ou {} se nunca salvo.
    """
    db = SessionLocal()
    try:
        row = db.query(DistributionSettings).filter(DistributionSettings.id == "default").first()
        if not row:
            return {}
        return {"enabled": bool(row.enabled), "interval_hours": int(row.interval_hours or 6)}
    except Exception as e:
        print(f"[Database] Erro ao ler agendador de distribuição: {e}")
        return {}
    finally:
        db.close()


def save_db_distribution_settings(enabled: bool, interval_hours: int) -> bool:
    """Persiste o estado do agendador de distribuição social (linha única,
    upsert idempotente — seguro sob concorrência)."""
    db = SessionLocal()
    try:
        row = db.merge(DistributionSettings(
            id="default",
        ...
        db.rollback()
        print(f"[Database] Erro ao salvar agendador de distribuição: {e}")
        return False
    finally:
        db.close()


def update_db_user_plan_by_email(email: str, plan: str, customer_id: str = None, subscription_id: str = None, expires_at: datetime = None) -> bool:
    db = SessionLocal()
    try:
