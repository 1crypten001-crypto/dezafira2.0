...
# ─── Cascata LLM centralizada em agents/llm.py (query_llm: OpenRouter → Gemini → NVIDIA → HuggingFace → DeepSeek) ───
...
async def _call_llm(system_prompt: str, user_prompt: str,
    ...
    """Chama LLM com a CASCATA UNICA do sistema (agents.llm.query_llm).

    Fonte da verdade: agents/llm.py
      OpenRouter -> Gemini -> NVIDIA NIM -> HuggingFace -> DeepSeek

    agents.llm retorna ERROR_PREFIX em falha total; este delegador
    preserva o comportamento historico de _call_llm (lancar RuntimeError)
    para os callers que dependem de try/except como fallback de secao.
    """
    from agents.llm import ERROR_PREFIX, query_llm
...
- O artigo terá ~{min(target_words, 1500)} palavras no total.
...
- Idioma: {language}
- Keywords: {keywords if keywords else topic}
- O artigo terá ~{min(target_words, 1500)} palavras no total (MAXIMO 1500)
- NAO repita a mesma frase, paragrafo ou ideia mais de uma vez no artigo
- LIMITE o artigo completo a 1300-1500 palavras (NUNCA ultrapasse 1500)
