...
def generate_article_view(slug: str, blog_info: dict, post: dict, related_posts: list = None) -> str:
    ...
    # Newsletter inline — captura real de leads (POST para /api/v1/newsletter/subscribe)
    newsletter_html = f"""
    <aside class="newsletter-inline scroll-fade" aria-label="Newsletter">
      ...
      <form class="newsletter-form" id="newsletter-form-{esc(post['id'])}" onsubmit="return submitNewsletter(this, '{esc(post['id'])}')">
        <input type="email" name="email" placeholder="Seu melhor e-mail" required aria-label="Email">
        <button type="submit">Inscrever</button>
      </form>
      <div class="newsletter-msg" id="newsletter-msg-{esc(post['id'])}" style="display:none;margin-top:10px;font-size:13px;font-weight:600"></div>
    </aside>
    <script>
    function submitNewsletter(form, pid) {{
      var input = form.querySelector('input[name=email]');
      var msg = document.getElementById('newsletter-msg-' + pid);
      ...
      fetch('/api/v1/newsletter/subscribe', {{
        ...
        body: JSON.stringify({{ email: input.value.trim(), source: 'blog' }})
      }}).then(function(r) {{ return r.json(); }}).then(function(d) {{
        msg.style.display = 'block';
        msg.style.color = '#16a34a';
        msg.textContent = d.success ? '✅ Inscrição confirmada! Verifique sua caixa de entrada.' : '⚠️ ' + (d.detail || 'Erro ao inscrever');
        if(d.success) form.querySelector('input').value = '';
    ...
    if brand_config and isinstance(brand_config.get("cta"), dict):
        ...
        c_title = esc(cta.get("title", "Quer ir além?"))
        c_text = esc(cta.get("text", "Acesse o conteúdo completo e comece hoje."))
        c_button = esc(cta.get("button", "Quero acessar"))
        c_url = esc(cta.get("url", "#"))
        c_emoji = esc(cta.get("emoji", "🚀"))
        if c_url and c_url != "#":
            cta_html = f"""
    <div class="cta-box scroll-fade" style="margin:32px 0;padding:28px 24px;border-radius:16px;background:linear-gradient(135deg,rgba(var(--primary-rgb,99,102,241),.12),rgba(var(--primary-rgb,99,102,241),.04));border:1px solid rgba(var(--primary-rgb,99,102,241),.25);text-align:center">
      <div style="font-size:34px;margin-bottom:8px">{c_emoji}</div>
      <h3 style="margin:0 0 8px;font-size:19px;font-weight:700;color:var(--dark,#0f172a)">{c_title}</h3>
      <p style="margin:0 auto 18px;max-width:520px;font-size:14px;line-height:1.6;color:var(--text-light,#64748b)">{c_text}</p>
      <a href="{c_url}" style="display:inline-block;padding:13px 30px;border-radius:10px;background:var(--primary,#6366f1);color:#fff;font-weight:700;font-size:15px;text-decoration:none;box-shadow:0 6px 18px rgba(var(--primary-rgb,99,102,241),.35);transition:transform .15s ease">{c_button}</a>
    </div>"""
