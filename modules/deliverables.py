...
async def create_deliverable_app_for_channel(channel_id: str, name: str, nicho: str, slug: str, checkout_url: str = "") -> dict:
    """Gera o conteúdo da IA e salva no banco de dados.

    checkout_url: URL opcional do checkout do Clube (produto/landing). Quando
    presente, o quiz é grátis até o final e o botão final redireciona para lá
    (funil: quiz → checkout do Clube) em vez de cobrar no próprio app.
    """
    import uuid
    ...
    if checkout_url and checkout_url.strip():
        config["checkout_url"] = checkout_url.strip()
        # Com checkout externo, o app não cobra — o preço vira mera exibição
        config.setdefault("pricing", {})
        config["pricing"]["checkout_external"] = True
