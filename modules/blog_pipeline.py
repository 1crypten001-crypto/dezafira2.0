...
async def get_reddit_questions(niche: str, lang: str = "pt") -> list:
    ...
    async def _collect_paa(term: str):
        """Coleta PAA real da SERP.

        A chave correta do bridge é 'people_also_ask' (não 'paa') — sem esse
        fix o PAA real do Obscura/Bing/DDG era descartado e a pipeline caía
        no fallback genérico (dores falsas). Também guarda títulos reais da
        SERP como evidência para o tier LLM."""
        nonlocal questions
        try:
            serp_data = await get_serp_with_fallback(term, lang=lang)
            if not isinstance(serp_data, dict):
                return
            paa_list = serp_data.get("people_also_ask") or []
            for q in paa_list:
                clean_q = str(q).strip()
                if clean_q and len(clean_q) > 10 and clean_q not in questions:
                    questions.append(clean_q)
            evidence_titles.extend((serp_data.get("titles") or [])[:10])
        except Exception as e:
            print(f"[Seu Reddit] Aviso na busca SERP/PAA de '{term}': {e}")

    # 1) PAA real do Google/fallback rotativo (Bing/DDG/Ecosia) com o termo simplificado
    await _collect_paa(short_niche)
    # 1b) Se veio pouco PAA, busca variações com intenção de dúvida real
    if len(questions) < 5:
        for extra in (f"{short_niche} dúvidas", f"como {short_niche}", f"melhor {short_niche}"):
            await _collect_paa(extra)
            if len(questions) >= 5:
                break
    if questions:
        print(f"[Seu Reddit] Encontradas {len(questions)} perguntas reais no PAA da SERP!")
    ...
    if len(questions) < 5:
        try:
            ...
            # Evidência real da SERP como contexto (títulos de resultados reais)
            evidence = " | ".join(evidence_titles[:12]) if evidence_titles else "(sem resultados reais — use seu conhecimento do nicho)"
            prompt = (
                f"Você é um pesquisador comportamental. O usuário quer criar um blog sobre: '{niche}'.\n"
                f"Estas são frases/títulos REAIS que apareceram na SERP (Google/Bing/DDG) para esse nicho:\n"
                f"{evidence}\n\n"
                f"Gere 10 perguntas e dúvidas REAIS, naturais e específicas que o público pesquisa ou discute, "
                f"fundamentadas NESSE material real (não invente temas fora dele).\n"
                f"REGRAS VITAIS:\n"
                f"- Perguntas naturais e fluidas em português, como alguém digitaria na busca.\n"
                f"- NUNCA repita a frase '{niche}' dentro das perguntas.\n"
                f"- PROIBIDO frases genéricas tipo 'quais são os mistérios de...', 'o que a ciência diz...', "
                f"'fatos surpreendentes...'. Seja específico: mencione o problema real da pessoa "
                f"(ex: 'como emagrecer sem academia', 'jejum intermitente funciona para iniciantes?').\n"
                f"- Idioma: {lang}\n"
                f"\n"
                f"Retorne APENAS um JSON no formato de array de strings: [\"pergunta 1\", \"pergunta 2\", ...]"
            )
            # _call_llm(system_prompt, user_prompt) — ordem dos args é crítica
            llm_res = await _call_llm("Retorne apenas JSON válido.", prompt)
            if llm_res:
    ...
    # 3) Último recurso: fallback genérico (SÓ se SERP E LLM falharem — o log
    #    avisa para detectarmos queda de qualidade das fontes de dor)
    if not questions:
        print(f"[Seu Reddit] AVISO: FALLBACK GENÉRICO usado para '{niche}' (SERP e LLM falharam) — dores podem não ser reais!")
        questions = [
...
async def get_youtube_pains(niche: str, lang: str = "pt") -> list:
    """Coleta dores REAIS que o público pesquisa no YouTube e no Google.

    Fontes (frases exatas que as pessoas digitam, não geradas):
      1. Autocomplete do YouTube (suggestqueries.youtube.com) — API pública
      2. Autocomplete do Google (get_google_suggestions) — API pública
      3. Títulos reais de vídeos do YouTube — via Obscura (renderiza JS real)
    LLM só enriquece se veio muito pouco, usando as fontes reais como base.
    """
    ...
    def _add(text, source):
        t = str(text).strip()
        if not t or len(t) < 8 or len(t) > 160:
            return
        key = t.lower()
        if key in seen:
            return
        seen.add(key)
        pains.append({"text": t, "source": source})

    # 1) Autocomplete REAL do YouTube (API pública, sem browser)
    try:
        import urllib.parse
        import httpx
        url = (
            "https://suggestqueries.youtube.com/complete/search?"
            "client=firefox&ds=yt&hl=pt-BR&q=" + urllib.parse.quote(niche[:60])
        )
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code == 200:
                ...
                if isinstance(data, list) and len(data) > 1:
                    for item in data[1]:
                        _add(item, "youtube_autocomplete")
    except Exception as e:
        print(f"[Seu YouTube] Aviso no autocomplete do YouTube: {e}")

    # 2) Autocomplete REAL do Google (API pública)
    try:
        from services.obscura_bridge import get_google_suggestions
        for sug in await get_google_suggestions(niche, lang):
            _add(sug, "google_suggest")
    except Exception as e:
        print(f"[Seu YouTube] Aviso no autocomplete do Google: {e}")

    # 3) Títulos reais de vídeos do YouTube (via Obscura — renderiza JS de verdade)
    try:
        from research.spiders.youtube_search import YouTubeSearchSpider
        videos = await YouTubeSearchSpider().search(niche, limit=12)
        for v in videos:
            title = v.get("title", "")
            if title:
                _add(title, "youtube_title")
    except Exception as e:
        print(f"[Seu YouTube] Aviso na busca de vídeos: {e}")

    # 4) Enriquecimento via LLM só se veio MUITO pouco — e com base nas fontes reais
    if len(pains) < 5:
        try:
            from modules.blog_writer import _call_llm
            real = " | ".join(p["text"] for p in pains[:20]) if pains else "(nenhuma frase real coletada)"
            prompt = (
                f"Você é um pesquisador de audiência. Coletamos estas frases REAIS de busca "
                f"(YouTube/Google) sobre '{niche}':\n{real}\n\n"
                f"Com base NESSE material, liste até 10 dores, dúvidas e desejos do público "
                f"como frases curtas e naturais (o que a pessoa digitaria na busca).\n"
                f"REGRAS: nunca repita o nicho '{niche}' literalmente; nada de frases genéricas "
                f"do tipo 'quais são os mistérios de...'; use palavras que as pessoas realmente usam.\n"
                f"Retorne APENAS JSON: [\"frase 1\", \"frase 2\", ...]"
            )
            raw = await _call_llm("Retorne apenas JSON válido.", prompt)
            if raw:
                clean = raw.strip()
                if "```json" in clean:
                ...
                if isinstance(parsed, list):
                    for item in parsed:
                        _add(item, "llm_from_real_searches")
        except Exception as e_llm:
            print(f"[Seu YouTube] Falha no enriquecimento LLM: {e_llm}")

    sources = ", ".join(sorted(set(p["source"] for p in pains)))
    print(f"[Seu YouTube] {len(pains)} dores reais coletadas (fontes: {sources})")
    return pains[:20]
...
class BlogMacroPipeline:
    ...
    async def _phase_arquitetura(self):
        ...
        try:
            ...
            # ─── FASE C2: Dores reais do YouTube + Google ───
            try:
                yt_pains = await get_youtube_pains(niche, self.state.language)
                self.state.youtube_search_pains = yt_pains
                self._update_macro(sid, "active", 24,
                    f"🎬 [Fase C2] {len(yt_pains)} dores reais do YouTube/Google coletadas!",
                    data={"youtube_search_pains": yt_pains})
            except Exception as e_yt:
                print(f"[Arquitetura] Erro nas dores do YouTube: {e_yt}")
    ...
    async def _phase_producao(self):
        ...
        # LiLi exige mín. 1200 palavras — mira em 1400+ para passar o gate com folga
        target_words = 1400
    ...
    async def _phase_entrega(self):
        ...
        # ─── DISTRIBUIÇÃO SOCIAL: divulgar artigos publicados ────────────
        self._update_macro(sid, "active", 78,
            "📤 Distribuindo artigos nas redes sociais (Pinterest/Twitter)...")
        try:
            from modules.distributor import distribuir_artigos_do_blog
            dist = await distribuir_artigos_do_blog(channel_id=channel_id or "default", limit=3)
            if dist.get("distribuidos") or dist.get("falhas"):
                self._update_macro(sid, "active", 82,
                    f"📤 Distribuição social: {dist.get('distribuidos',0)} enviados, {dist.get('falhas',0)} falhas")
            else:
                self._update_macro(sid, "active", 82,
                    "📤 Distribuição social: nenhuma plataforma configurada — pulado")
        except Exception as e:
            print(f"[Pipeline] Distribuição warning: {e}")
