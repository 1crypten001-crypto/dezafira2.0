...
let {
    ...
    form?: { success?: boolean; token?: string; error?: string };
  } = $props();
...
<div class="cli-page">
      ...
      <div class="token-display">
        <div class="token-field">
          <span class="token-value" class:revealed={showToken}>
            {#if showToken}
              {currentToken || t(lang, 'admin.cli.no_token')}
            {:else}
              {currentToken ? '••••••••••••••••••••••••••••••••••••••' : t(lang, 'admin.cli.no_token')}
            {/if}
          </span>
        ...
        {:else}
          <p class="token-hint">⚠️ Guarde este token em local seguro. Ao regenerar, o token anterior para de funcionar imediatamente.</p>
        {/if}
      </div>
  ...
  <section class="section">
    ...
    <div class="doc-card">
      <div class="doc-header">
        ...
        <span class="doc-desc">Listar todos os posts</span>
      </div>
      <div class="doc-body">
        <div class="code-block-wrapper">
          <div class="code-label">curl</div>
          <button class="copy-btn" onclick={() => copyCmd(`curl -H "Authorization: Bearer ${currentToken || 'SEU_TOKEN'}" ${baseUrl}/api/cli/posts`, 'get-posts')}>
  ...
  {baseUrl}/api/cli/posts</code></pre>
        ...
        <div class="response-example">
          ...
          <pre><code>{`{ "posts": [{ "id": 1, "title": "...", "slug": "...", "published": true, "pinterest_enabled": true, "categories": "Looks", ... }] }`}</code></pre>
        </div>
      </div>
    </div>
  ...
  ${baseUrl}/api/cli/posts/1`, 'edit-post-pub')}>
    ...
    <div class="doc-card">
      ...
      <div class="doc-body">
        <div class="code-block-wrapper">
          <div class="code-label">curl</div>
          <button class="copy-btn" onclick={() => copyCmd(`curl -X DELETE -H "Authorization: Bearer ${currentToken || 'SEU_TOKEN'}" ${baseUrl}/api/cli/posts/1`, 'delete-post')}>
