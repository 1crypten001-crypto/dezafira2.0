...
export default function FabricaMapasPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--ink)] text-[#f8fafc]">
      <header className="border-b border-[var(--surface)] bg-[var(--ink)] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/admin" className="text-sm text-[var(--text-dim)] hover:text-white">← Voltar ao Admin</Link>
          <span className="text-[#334155]">/</span>
          <h1 className="text-lg font-bold text-[var(--brand)]">🧠 Fábrica de Mapas Mentais</h1>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* Painel de Criação */}
        <div className="bg-[var(--ink)] border border-[var(--surface)] rounded-2xl p-6 shadow-xl space-y-6">
          <div>
          ...
          <form onSubmit={handleGenerate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-[var(--text-dim)] mb-1">NICHO OU TEMA PRINCIPAL</label>
                <input
                  type="text"
                  value={niche}
              ...
              <div>
                <label className="block text-xs font-bold text-[var(--text-dim)] mb-1">TÍTULO (OPCIONAL)</label>
                <input
                  type="text"
                  value={title}
              ...
              <div>
                <label className="block text-xs font-bold text-[var(--text-dim)] mb-1">PREÇO DA ASSINATURA (CENTAVOS)</label>
                <input
                  type="number"
                  value={priceCents}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-[var(--brand)] to-[var(--brand)] text-white font-extrabold py-3.5 rounded-xl text-sm shadow-lg hover:brightness-110 disabled:opacity-50 transition-all"
            >
        ...
        {progress && (
          <div className="bg-[var(--ink)] border border-[var(--surface)] rounded-2xl p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-mono text-[var(--brand)]">{progress.status === "completed" ? "100%" : progress.current_macro_stage || "Iniciando"}</span>
            </div>
            <div className="w-full bg-[var(--surface)] rounded-full h-2">
              <div
                className="h-2 rounded-full bg-gradient-to-r from-[var(--brand)] to-[#38bdf8] transition-all duration-500"
                style={{ width: progress.status === "completed" ? "100%" : "50%" }}
              />
            </div>
            <p className="text-xs text-[var(--text-dim)]">{progress.error || "Aguardando conclusão de escrita do grafo..."}</p>
          </div>
        )}

        {/* Lista de Mapas Mentais Criados */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 bg-[var(--ink)] border border-[var(--surface)] rounded-2xl p-6 shadow-xl h-[600px] flex flex-col">
            <h2 className="text-lg font-bold text-[#f1f5f9] mb-4">Mapas Disponíveis ({mindmaps.length})</h2>
            <div className="flex-1 overflow-y-auto space-y-3 pr-1">
              {loadingList ? (
                <div className="text-sm text-[var(--text-dim)]">Carregando mapas...</div>
              ) : mindmaps.length === 0 ? (
                <div className="text-sm text-[var(--text-dim)]">Nenhum mapa criado ainda.</div>
              ) : (
                mindmaps.map((m) => (
                  <div
                    key={m.id}
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-all ${
                      selectedMap?.id === m.id ? "bg-[var(--surface)] border-[var(--brand)]" : "bg-[var(--surface)] border-[var(--surface)] hover:border-gray-700"
                    }`}
                  >
                    ...
                    <p className="text-xs text-[var(--brand)] mt-1">{m.niche}</p>
                    <div className="flex justify-between items-center mt-3">
                      ...
                      <div className="flex gap-2 items-center">
                        <a
                          ...
                          target="_blank"
                          className="text-[10px] text-[var(--text-dim)] hover:underline"
                        ...
                        <button
                          onClick={async (e) => {
                            ...
                            const token = typeof window !== "undefined" ? localStorage.getItem("dz_token") : "";
                            try {
                              await fetch(`${API_URL}/api/v1/mindmaps/${m.id}/publish`, {
                                method: "POST",
                                headers: token ? { Authorization: `Bearer ${token}` } : {}
                              });
                              alert("Liberado no Club!");
                            } catch (err) {
                              console.error(err);
                            }
                          }}
                          className="text-[10px] text-[var(--brand)] font-bold hover:underline"
                        >
                          Liberar no Club →
                        </button>
                      </div>
          ...
          <div className="md:col-span-2 bg-[var(--ink)] border border-[var(--surface)] rounded-2xl p-6 shadow-xl h-[600px] flex flex-col">
            {selectedMap ? (
              <div className="flex flex-col h-full space-y-4">
                <div className="border-b border-[var(--surface)] pb-4">
                  <h2 className="text-xl font-extrabold text-[#f1f5f9]">{selectedMap.title}</h2>
                  <p className="text-xs text-[var(--text-dim)] mt-1">Nicho: {selectedMap.niche} | Estilo: {selectedMap.style_id}</p>
                </div>
                <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                  <h3 className="text-sm font-bold text-[var(--brand)]">Hierarquia e Quizzes (Visualização Rápida)</h3>
                  {selectedMap.map_json ? (
                    <div className="bg-[var(--surface)] border border-[var(--surface)] p-4 rounded-xl font-mono text-xs overflow-x-auto text-gray-300">
                      <pre>{JSON.stringify(JSON.parse(selectedMap.map_json), null, 2)}</pre>
                    </div>
                  ) : (
                    <div className="text-sm text-[var(--text-dim)] italic">Estrutura JSON não gerada ou corrompida.</div>
                  )}
                </div>
              </div>
            ) : (
