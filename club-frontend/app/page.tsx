...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#080d19] text-white selection:bg-purple-500/30 selection:text-white">
      {/* Navbar */}
      <nav className="fixed top-0 w-full z-50 bg-[#080d19]/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm">D</div>
            <span className="text-lg font-bold text-white tracking-wide">Dezafira</span>
          </div>
          <div className="flex items-center gap-4">
            {user ? (
              <Link href="/painel" className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium transition-all duration-200 shadow-md shadow-purple-600/10">
                Painel
              </Link>
            ) : (
              <Link href="/auth/login" className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium transition-all duration-200 shadow-md shadow-purple-600/10">
                Entrar
              </Link>
            )}
          </div>
      ...
      <section className="relative pt-36 pb-24 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[400px] bg-gradient-to-br from-purple-600/15 via-indigo-600/5 to-transparent rounded-full blur-[140px]" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold mb-6 leading-tight tracking-tight">
            ...
            <span className="bg-gradient-to-r from-purple-400 via-purple-300 to-indigo-400 bg-clip-text text-transparent">Sob Controle</span>
            <br />
          ...
          {!user ? (
            <div className="flex gap-4 justify-center">
              <Link href="/auth/login" className="px-8 py-4 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-lg font-semibold transition-all hover:shadow-lg hover:shadow-purple-500/25">
                Entrar no Painel
              </Link>
            </div>
          ) : (
      ...
      <section className="max-w-6xl mx-auto px-4 py-20 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { icon: "📝", title: "Fábrica Blog", desc: "Crie e publique posts inteligentes com SEO otimizado automaticamente.", color: "from-purple-500/10 to-indigo-500/5", border: "border-purple-500/10" },
            { icon: "📚", title: "Fábrica Ebook", desc: "Gere ebooks profissionais com layout pronto para download.", color: "from-indigo-500/10 to-blue-500/5", border: "border-indigo-500/10" },
            ...
            <div key={f.title} className={`relative group rounded-2xl border ${f.border} bg-gradient-to-br ${f.color} p-8 hover:scale-[1.02] hover:border-purple-500/20 transition-all duration-300`}>
      ...
      {/* CTA Section */}
      <section className="max-w-6xl mx-auto px-4 py-20 relative z-10">
        <div className="relative rounded-3xl bg-gradient-to-br from-purple-600/20 via-indigo-600/10 to-purple-600/20 border border-purple-500/20 p-12 text-center overflow-hidden">
      ...
      <footer className="border-t border-white/5 py-8 relative z-10">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between text-sm text-gray-500">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs">D</div>
            <span>Dezafira</span>
          </div>
