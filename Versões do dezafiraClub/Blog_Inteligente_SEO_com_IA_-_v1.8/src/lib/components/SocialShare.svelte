<script lang="ts">
  ...
  // Obter URL atual se não especificada
  ...
  let encodedDesc = $derived(encodeURIComponent(description || ''));
...
<div class="social-share size-{size}">
  {#each shareLinks as link}
    <a 
      ...
      aria-label={t(lang, 'share.aria', { name: link.name })}
      title={link.name}
    >
      ...
      {#if showLabels}
        <span class="share-label">{link.name}</span>
      {/if}
    </a>
  {/each}
  ...
  <button 
    ...
    onclick={copyLink}
    aria-label={t(lang, 'share.copy_link')}
    title={t(lang, 'share.copy_link')}
  >
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    {#if showLabels}
      <span class="share-label">{t(lang, 'share.copy')}</span>
    {/if}
  </button>
</div>
...
<style>
  ...
  .share-btn:hover {
    ...
    box-shadow: 0 4px 12px color-mix(in srgb, var(--share-color) 30%, transparent);
  }
