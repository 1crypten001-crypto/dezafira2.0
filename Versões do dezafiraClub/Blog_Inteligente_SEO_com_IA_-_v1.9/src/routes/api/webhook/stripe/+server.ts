    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (type === 'premium_subscription') {
        ...
        const purchase = await queryOne(
          'SELECT pp.*, p.resource_type, p.bundle_items FROM product_purchases pp LEFT JOIN products p ON pp.product_id = p.id WHERE pp.stripe_session_id = ?',
          [sessionId]
        );
        if (purchase) {
          ...
          if (newStatus === 'completed' && purchase.bundle_items) {
            ...
            const unlocked = await unlockBundleForUser(purchase.product_id, purchase.user_id, { stripeSessionId: sessionId });
            if (unlocked > 0) console.log(`[STRIPE WEBHOOK] Bundle ${purchase.product_id}: ${unlocked} produtos desbloqueados`);
          }
        }
