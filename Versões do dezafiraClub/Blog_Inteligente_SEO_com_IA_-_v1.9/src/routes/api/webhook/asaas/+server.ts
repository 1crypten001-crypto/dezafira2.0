  ...
  try {
    ...
    if (event === 'PAYMENT_RECEIVED' || event === 'PAYMENT_CONFIRMED') {
      ...
      if (payment.subscription) {
        ...
        const purchase = await queryOne('SELECT pp.*, p.resource_type, p.bundle_items FROM product_purchases pp LEFT JOIN products p ON pp.product_id = p.id WHERE pp.asaas_payment_id = ?', [payment.id]);
        if (purchase) {
          ...
          // Combo/pacote nativo: desbloqueia os produtos incluídos no bundle
          if (newStatus === 'completed' && purchase.bundle_items) {
            const { unlockBundleForUser } = await import('$lib/server/database');
            const unlocked = await unlockBundleForUser(purchase.product_id, purchase.user_id, { asaasPaymentId: payment.id });
            if (unlocked > 0) console.log(`[ASAAS WEBHOOK] Bundle ${purchase.product_id}: ${unlocked} produtos desbloqueados`);
          }
        } else {
