  ...
  <div class="product-main-card">
    <div class="product-grid">
      ...
      <div class="product-details-column">
        ...
        {#if data.bundleProducts.length > 0}
          <div class="bundle-box" style="background: linear-gradient(135deg, #fef3c7, #fde68a); border: 1px solid #f59e0b; border-radius: 12px; padding: 1rem 1.1rem; margin-bottom: 1.25rem;">
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.6rem;">
              <span style="background: #f59e0b; color: #fff; font-size: 0.65rem; font-weight: 800; padding: 2px 6px; border-radius: 4px; letter-spacing: 0.05em; text-transform: uppercase;">📦 PACOTE / COMBO</span>
            </div>
            <p style="font-size: 0.8rem; font-weight: 700; color: #92400e; margin: 0 0 0.5rem;">Este pacote inclui {data.bundleProducts.length} produtos:</p>
            <ul style="list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.4rem;">
              {#each data.bundleProducts as item}
                <li style="display: flex; align-items: center; gap: 0.6rem; font-size: 0.82rem; color: #78350f;">
                  {#if item.image_url}
                    <img src={optimizeImageUrl(item.image_url, 48, 48)} alt={item.name} style="width: 34px; height: 34px; object-fit: cover; border-radius: 8px; border: 1px solid rgba(245,158,11,0.5);" />
                  {:else}
                    <span style="width: 34px; height: 34px; border-radius: 8px; background: rgba(245,158,11,0.15); display: inline-flex; align-items: center; justify-content: center;">🎁</span>
                  {/if}
