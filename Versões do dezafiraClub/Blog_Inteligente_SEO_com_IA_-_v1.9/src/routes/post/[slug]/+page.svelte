<script lang="ts">
  ...
  function getYouTubeId(url: string | null | undefined): string | null {
    ...
    // Se a pessoa colou o código <iframe> completo, extrai a URL do atributo src
    const iframeMatch = str.match(/src=["']([^"']+)["']/i);
    if (iframeMatch) {
      str = iframeMatch[1];
    }

    // ID direto de 11 caracteres (ex: "dQw4w9WgXcQ")
    if (/^[a-zA-Z0-9_-]{11}$/.test(str)) {
      return str;
    }

    // Padrões de URLs do YouTube (watch, embed, shorts, live, youtu.be, etc.)
    const patterns = [
    ...
    // Fallback: extrai qualquer ID de 11 caracteres caso seja uma URL do youtube
  ...
  const processedContent = $derived.by(() => {
    ...
    if (videoId) {
      html = html.replace(
        /<div class="video-container"[^>]*>\s*<\/div>/gi,
        `<div class="video-container" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin-bottom:2rem;border-radius:12px;">
          <iframe src="https://www.youtube.com/embed/${videoId}" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;border-radius:12px;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen title="Vídeo do Artigo"></iframe>
        </div>`
      );
    }
    return html;
