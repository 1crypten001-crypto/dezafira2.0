  ...
  clientPromise = (async () => {
    ...
    if (USE_TURSO) {
      ...
      localClient = {
        ...
        batch: async (statements: { sql: string; args?: any[] }[]) => {
          const executeBatch = sqliteDb.transaction((items: { sql: string; args?: any[] }[]) =>
            items.map(({ sql, args = [] }) => {
              const stmt = sqliteDb.prepare(sql);
...
export async function initDatabase() {
  ...
  // category_id + migração de categorias — DEPOIS da tabela products existir
  // (banco fresco quebrava porque o ALTER rodava antes do CREATE).
  try {
  ...
  // Stripe session em product_purchases — DEPOIS do CREATE (banco fresco)
  try {
  ...
  // Migration: combo/pacote nativo — produtos incluídos (JSON array de ids)
  try {
    await exec(`ALTER TABLE products ADD COLUMN bundle_items TEXT`);
  } catch (_) { /* column already exists */ }
  ...
  // Stripe session em course_purchases — DEPOIS do CREATE (banco fresco)
  try {
  ...
  try {
    ...
    console.error('Error creating community tables:', e);
  }
  ...
  // 4. Semear usuário admin. ADMIN_PASSWORD (env) é a FONTE DA VERDADE:
  //    se definido, cria OU sincroniza o hash a cada boot — corrige login
  //    quando a senha muda no env mas o usuário já existe no banco.
  try {
    ...
    if (adminPass) {
      ...
      if (userExists) {
        await run('UPDATE users SET password = ? WHERE username = ?', [hashedPassword, adminUser]);
        console.log(`Admin user "${adminUser}" password synced from ADMIN_PASSWORD env.`);
      } else {
        await run('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', [adminUser, hashedPassword, 'admin']);
        console.log('Admin user created successfully.');
      }
    } else if (!userExists) {
      // Sem ADMIN_PASSWORD no env (legado): cria apenas na primeira montagem com fallback
      console.log('Seeding default admin user (first run, no ADMIN_PASSWORD set)...');
      const bcrypt = await import('bcryptjs');
      const hasher = bcrypt.default ? bcrypt.default : bcrypt;
      const hashedPassword = await hasher.hash('Xk9mP2vL7qR4StrongPass!', 10);
      await run('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', [adminUser, hashedPassword, 'admin']);
      console.log('Admin user created successfully.');
    }
  } catch (e) {
...
export async function runBatch(statements: { sql: string; args?: any[] }[]) {
  ...
  if (typeof db.batch !== 'function') {
    throw new Error('O driver de banco atual não oferece suporte a transações em lote.');
  }
...
export async function getPostsAdminPage(options: { page: number; limit: number; search?: string }) {
  const offset = (options.page - 1) * options.limit;
  const search = options.search?.trim() || '';
  const where = search ? 'WHERE p.title LIKE ? OR p.excerpt LIKE ?' : '';
  const searchParams = search ? [`%${search}%`, `%${search}%`] : [];
  const totalRow = await queryOne(`SELECT COUNT(*) AS count FROM posts p ${where}`, searchParams);
  const posts = await query(
    `SELECT p.id, p.title, p.slug, p.excerpt, p.cover_image, p.published,
     ...
     ORDER BY p.created_at DESC
     LIMIT ? OFFSET ?`,
    [...searchParams, options.limit, offset]
  );
...
export async function getExistingCategoryIds(categoryIds: number[]): Promise<number[]> {
  const uniqueIds = [...new Set(categoryIds)];
  if (uniqueIds.length === 0) return [];
  const placeholders = uniqueIds.map(() => '?').join(', ');
  const rows = await query(`SELECT id FROM categories WHERE id IN (${placeholders})`, uniqueIds);
...
const CLI_TOKEN_LIFETIME_MS = 90 * 24 * 60 * 60 * 1000;
...
export async function getCLITokenStatus(): Promise<CLITokenStatus> {
  const [hashRow, expiryRow, legacyRow] = await Promise.all([
    queryOne('SELECT value FROM settings WHERE key = ?', ['cli_token_hash']),
    queryOne('SELECT value FROM settings WHERE key = ?', ['cli_token_expires_at']),
    queryOne('SELECT value FROM settings WHERE key = ?', ['cli_token'])
  ]);
  const expiresAt = expiryRow?.value ? Number(expiryRow.value) : null;
  const hashConfigured = typeof hashRow?.value === 'string' && hashRow.value.length > 0;
  return {
    configured: hashConfigured || Boolean(legacyRow?.value),
    expiresAt: Number.isFinite(expiresAt) ? expiresAt : null,
    legacy: !hashConfigured && Boolean(legacyRow?.value)
  };
}

export async function regenerateCLIToken(): Promise<string> {
  const token = `blog_${crypto.randomBytes(32).toString('hex')}`;
  const expiresAt = Date.now() + CLI_TOKEN_LIFETIME_MS;
  await runBatch([
    { sql: 'INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', args: ['cli_token_hash', hashCLIToken(token)] },
    { sql: 'INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', args: ['cli_token_expires_at', String(expiresAt)] },
    { sql: 'DELETE FROM settings WHERE key = ?', args: ['cli_token'] }
  ]);
  return token;
}
...
export async function validateCLIToken(token: string): Promise<boolean> {
  ...
  const [hashRow, expiryRow] = await Promise.all([
    queryOne('SELECT value FROM settings WHERE key = ?', ['cli_token_hash']),
    queryOne('SELECT value FROM settings WHERE key = ?', ['cli_token_expires_at'])
  ]);
  ...
  if (hashRow?.value) {
    ...
    return safeHashEquals(hashCLIToken(token), String(hashRow.value));
  }

  // Backward-compatible one-time migration for tokens stored by versions <= 1.8.
  const legacyToken = await getCLIToken();
  if (!legacyToken || !safeHashEquals(hashCLIToken(token), hashCLIToken(legacyToken))) return false;
...
export interface Product {
  ...
  // Esteira de produtos: upsell/downsell pós-compra (Dezafira)
...
export async function createProduct(prod: {
  ...
  return run(
    ...
    [
      ...
      prod.bundle_items && prod.bundle_items.length ? JSON.stringify(prod.bundle_items) : null
    ]
  );
}
...
/**
 * Retorna os ids dos produtos incluídos em um combo/pacote (bundle_items).
 */
export async function getProductBundleItems(productId: number): Promise<number[]> {
  const row = await queryOne('SELECT bundle_items FROM products WHERE id = ?', [productId]);
  if (!row || !row.bundle_items) return [];
  try {
    const arr = JSON.parse(row.bundle_items);
    return Array.isArray(arr) ? arr.map((n: any) => Number(n)).filter((n: number) => Number.isInteger(n) && n > 0) : [];
  } catch (_) {
    return [];
  }
}
...
/**
 * Localiza o combo/pacote que inclui o produto informado (reverse lookup).
 * Usado no pós-compra: quem comprou só o principal recebe a oferta do pacote completo.
 */
export async function findBundleForProduct(productId: number): Promise<any | null> {
  const rows = await query(
    "SELECT * FROM products WHERE bundle_items IS NOT NULL AND bundle_items != '' AND price_cents > 0"
  );
  for (const r of rows) {
    try {
      const items = JSON.parse(r.bundle_items);
      if (Array.isArray(items) && items.map((n: any) => Number(n)).includes(Number(productId))) {
        return r;
      }
    } catch (_) { /* bundle_items inválido — ignora */ }
  }
  return null;
}

/**
 * Retorna os produtos incluídos em um combo/pacote (com nome, slug, imagem e preço).
 */
export async function getBundleProducts(bundleProductId: number): Promise<any[]> {
  const itemIds = await getProductBundleItems(bundleProductId);
...
/**
 * Desbloqueia os produtos de um combo/pacote para o usuário — após o pagamento
 * do bundle ser confirmado, cada produto incluído ganha um registro de compra
 * 'completed', aparecendo na área de membros do comprador.
 */
export async function unlockBundleForUser(
  bundleProductId: number,
  userId: number,
  opts: { asaasPaymentId?: string; stripeSessionId?: string } = {}
): Promise<number> {
  const itemIds = await getProductBundleItems(bundleProductId);
  let unlocked = 0;
  for (const itemId of itemIds) {
    if (itemId === bundleProductId) continue;
    const existing = await hasUserPurchasedProduct(userId, itemId);
    if (existing) continue;
    const item = await getProductById(itemId);
    await createProductPurchase({
      userId,
      productId: itemId,
      productNameSnapshot: item ? item.name : null,
      priceCents: 0,
      status: 'completed',
      asaasPaymentId: opts.asaasPaymentId || null,
...
export async function getLandingPagesPage({ page = 1, limit = 25, search = '' } = {}): Promise<any> {
  const offset = (page - 1) * limit;
  const term = `%${search}%`;
  const where = search ? 'WHERE title LIKE ? OR slug LIKE ?' : '';
  const params = search ? [term, term] : [];
  const totalRow = await queryOne(`SELECT COUNT(*) AS count FROM landing_pages ${where}`, params);
  const pages = await query(
    `SELECT * FROM landing_pages ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
    [...params, limit, offset]
  );
  const total = Number(totalRow?.count || 0);
  return { pages, page, limit, total, totalPages: Math.ceil(total / limit) };
}

export async function getLandingBuilderPosts(): Promise<any[]> {
  return await query(
    `SELECT id, title, slug, excerpt, cover_image, created_at
     FROM posts WHERE published = 1 ORDER BY created_at DESC LIMIT 100`
  );
}
...
/* ============================================================================
 * COMUNIDADE VIP DE ASSINANTES (TOPICS, COMMENTS & LIKES)
 * ============================================================================ */

export async function getCommunityTopics(tenantId = 'default', category?: string, currentUserId?: number) {
  let sql = `
    SELECT 
      t.*,
      u.username as user_email,
      u.role as user_role,
      (SELECT COUNT(*) FROM community_likes l WHERE l.topic_id = t.id) as likes_count,
      (SELECT COUNT(*) FROM community_comments c WHERE c.topic_id = t.id) as comments_count
      ${currentUserId ? `, (SELECT COUNT(*) FROM community_likes cl WHERE cl.topic_id = t.id AND cl.user_id = ${Number(currentUserId)}) > 0 as user_has_liked` : ', 0 as user_has_liked'}
  ...
  return topics.map((t: any) => ({
    ...t,
    user_has_liked: Boolean(t.user_has_liked),
    is_pinned: Boolean(t.is_pinned)
  }));
}

export async function getCommunityTopicById(id: number, currentUserId?: number) {
  const sql = `
    SELECT 
      t.*,
      u.username as user_email,
    ...
    WHERE t.id = ?
  `;
  const topic = await queryOne(sql, [id]);
  if (!topic) return null;
  return {
    ...topic,
    user_has_liked: Boolean(topic.user_has_liked),
    is_pinned: Boolean(topic.is_pinned)
  };
}

export async function createCommunityTopic(data: {
  tenantId?: string;
  userId: number;
  title: string;
  content: string;
  ...
  const result = await run(
    `INSERT INTO community_topics (tenant_id, user_id, title, content, category, is_pinned) VALUES (?, ?, ?, ?, ?, ?)`,
    [tenantId, data.userId, data.title, data.content, category, isPinned]
  );
  return result?.lastInsertRowid ? Number(result.lastInsertRowid) : null;
}

export async function deleteCommunityTopic(id: number) {
  await run(`DELETE FROM community_topics WHERE id = ?`, [id]);
}

export async function togglePinCommunityTopic(id: number, isPinned: boolean) {
  await run(`UPDATE community_topics SET is_pinned = ? WHERE id = ?`, [isPinned ? 1 : 0, id]);
}

export async function getCommunityComments(topicId: number) {
  const sql = `
    SELECT 
      c.*,
      u.username as user_email,
  ...
  `;
  return await query(sql, [topicId]);
}
...
export async function createCommunityComment(data: {
  ...
  const result = await run(
    `INSERT INTO community_comments (topic_id, user_id, content) VALUES (?, ?, ?)`,
    [data.topicId, data.userId, data.content]
  );
  await run(
    `UPDATE community_topics SET comments_count = (SELECT COUNT(*) FROM community_comments WHERE topic_id = ?) WHERE id = ?`,
    [data.topicId, data.topicId]
  );
  return result?.lastInsertRowid ? Number(result.lastInsertRowid) : null;
}

export async function deleteCommunityComment(commentId: number, topicId: number) {
  await run(`DELETE FROM community_comments WHERE id = ?`, [commentId]);
  await run(
    `UPDATE community_topics SET comments_count = (SELECT COUNT(*) FROM community_comments WHERE topic_id = ?) WHERE id = ?`,
    [topicId, topicId]
  );
...
export async function toggleCommunityLike(topicId: number, userId: number) {
  const existing = await queryOne(
    `SELECT id FROM community_likes WHERE topic_id = ? AND user_id = ?`,
    [topicId, userId]
  );

  if (existing) {
    await run(`DELETE FROM community_likes WHERE topic_id = ? AND user_id = ?`, [topicId, userId]);
  } else {
    await run(`INSERT OR IGNORE INTO community_likes (topic_id, user_id) VALUES (?, ?)`, [topicId, userId]);
  }

  await run(
    `UPDATE community_topics SET likes_count = (SELECT COUNT(*) FROM community_likes WHERE topic_id = ?) WHERE id = ?`,
    [topicId, topicId]
  );
...
export async function getCommunityStats() {
  const totalTopics = await queryOne(`SELECT COUNT(*) as count FROM community_topics`);
  const totalComments = await queryOne(`SELECT COUNT(*) as count FROM community_comments`);
  const totalLikes = await queryOne(`SELECT COUNT(*) as count FROM community_likes`);
  return {
