...
const ALLOWED_TAGS = new Set([
  ...
  'div','span','small','sup','sub','mark','del','ins','svg','path','circle','rect','line','polyline','polygon',
  'iframe'
]);

const ALLOWED_PROTOCOLS = ['http:', 'https:', 'mailto:'];

const FORBIDDEN_TAGS = new Set(['script','style','form','input','button','select','textarea','object','embed','applet']);
...
const ALLOWED_ATTRS = new Set([
  'href','src','alt','title','class','style','target','rel','width','height','loading',
  'colspan','rowspan','role','aria-label','aria-hidden','viewbox','fill','stroke',
  'stroke-width','stroke-linecap','stroke-linejoin','d','x','y','x1','x2','y1','y2',
  'cx','cy','r','rx','ry','points','xmlns',
...
function cleanInlineStyle(value: string): string {
  if (/url\s*\(|expression\s*\(|@import|behavior\s*:|-moz-binding/i.test(value)) return '';
  ...
  try {
    ...
    while ((node = walker.nextNode() as Element | null)) {
      ...
      for (let i = attrs.length - 1; i >= 0; i--) {
        ...
        } else if (!ALLOWED_ATTRS.has(attr.name.toLowerCase()) && !attr.name.toLowerCase().startsWith('aria-')) {
          node.removeAttribute(attr.name);
