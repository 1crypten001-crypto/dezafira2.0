...
| **🌐 Chrome real como serviço** | `docker/chrome/Dockerfile` sobe o Chrome real headless (CDP 9223) como serviço no Railway (Root Directory `SniperVideoEngine/docker/chrome`) — o bridge tenta **Chrome primeiro** (`OBSCURA_CHROME_HOST`/`OBSCURA_CHROME_PORT`), senão Obscura; cadeia prod = local (Chrome → Google → fallback rotativo). Chrome 136+ ignora `--remote-debugging-address=0.0.0.0` → entrypoint usa **socat** (Chrome no loopback `OBSCURA_CHROME_INNER_PORT` default 9224 + socat expondo `0.0.0.0:9223 → 127.0.0.1:9224`) |
