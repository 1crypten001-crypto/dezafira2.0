    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard title="Receita" value={`R$ ${overview.revenue?.total_brl?.toFixed(2) || "0.00"}`} subtitle={`${overview.combos?.total} combos`} color="green" />
        </div>
      )}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 p-4 rounded-lg">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-400">Receita Total</span><span>R$ {overview?.revenue?.total_brl?.toFixed(2) || "0.00"}</span></div>
            <div className="flex justify-between"><span className="text-gray-400">Combos Vendidos</span><span>{overview?.combos?.total || 0}</span></div>
          </div>
        </div>
      </div>
    </div>
