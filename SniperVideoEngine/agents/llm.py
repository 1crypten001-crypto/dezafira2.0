...
async def _call_huggingface(key: str, system_prompt: str, user_prompt: str, temp: float, max_tok: int) -> Optional[str]:
    ...
    payload = {
        ...
        "parameters": {"temperature": temp, "max_new_tokens": min(max_tok, 4096), "return_full_text": False},
    }
    hf_models = ["mistralai/Mixtral-8x7B-Instruct-v0.1", "HuggingFaceH4/zephyr-7b-beta"]
...
async def _call_deepseek(key: str, messages: list, temp: float, max_tok: int) -> Optional[str]:
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        r = await client.post(
            ...
            json={"model": "deepseek-chat", "messages": messages, "temperature": temp, "max_tokens": max_tok},
            headers=headers,
        )
        if r.status_code == 200:
            return r.json()["choices"][0]["message"]["content"].strip()
