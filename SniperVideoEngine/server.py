...
async def run_blog_factory_endpoint(payload: dict):
    ...
    language = payload.get("language", "pt")
    target_articles = payload.get("target_articles", 3)  # Padrão: 3 artigos profundos
    ...
    # Armazena estado para consulta via GET
    ...
    async def _run_macro_with_ws(tid, bname, nic, lang, tgt):
        ...
        import asyncio
        def on_progress(pid, stage_id, progress, message, data):
            # Atualiza _macro_results em tempo real para o polling
            try:
            ...
            try:
                ...
                print(f"[WS] progress error: {e}")
        try:
            result = await _run_macro(blog_name=bname, niche=nic, language=lang, task_id=tid,
                                      target_articles=tgt, on_progress=on_progress)
        except Exception as e:
            import traceback
            print(f"[PIPELINE] ERRO na execucao: {e}")
            traceback.print_exc()
            _macro_results[tid] = {"status": "failed", "blog_name": bname,
                                   "niche": nic, "data": None, "error": str(e)}
            return
