...
- [x] **Custom domain criado no Railway (16/08)** — `1convite.com.br` → serviço `dezafiraadm` (porta 8080), id `1b5d315e`. Instruções de DNS fornecidas: **CNAME `1convite.com.br` → `yufqkp4n.up.railway.app`** (tráfego) + **TXT `railway-verify=49cc427d...`** (SSL). ⏳ **Falta o dono apontar o DNS** — verificado em 16/08: domínio está no **DNS da Hostinger (dns-parking)**, ainda resolve para `69.46.46.24` (hosting antigo do 1Convite, que segue no ar servindo o app original) e **não há CNAME nem TXT** nos registros públicos. O Railway já emitiu certificado (status `verified`, cert VÁLIDO — a configuração dos dois lados está pronta, só falta o DNS). Banco de produção já tem `miniapp_domains` = 1convite.com.br → miniapp 1convite (ativo); `RAILWAY_SERVICE_DEZAFIRAADM_URL=1convite.com.br`
