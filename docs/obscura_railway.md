...
## ⚠️ Requisito nº 1: MESMO projeto Railway do backend

O backend alcança o Obscura **somente via Private Networking**, que só existe
entre serviços do **mesmo projeto** (mesmo environment).

**Como conferir:** o sufixo do domínio gerado (ex.: `backend-production-92e1`
vs `obscura-production-9f18`) é o ID do environment — se os sufixos forem
diferentes, os serviços estão em projetos distintos e a rede privada **não
conecta** (o bridge veria `Name or service not known` ao resolver
`obscura.railway.internal`). Nesse caso, recrie o serviço Obscura no projeto
do backend (repo `dezafira2.0`, Root Directory `/docker/obscura`) e apague o
antigo para não pagar duas instâncias.

## 1) Subir o serviço no Railway

1. No **mesmo projeto Railway** do backend, crie um **segundo serviço**
2. No campo **Root Directory**, aponte para `docker/obscura`
   (repo novo `dezafira2.0` — no repo antigo era `SniperVideoEngine/docker/obscura`;
...
1. Crie um serviço apontando o **Root Directory** para
   `docker/chrome` (repo novo `dezafira2.0`; contém `Dockerfile` padrão que o
...
> ws://127.0.0.1:9223`). Por isso o entrypoint roda o Chrome no loopback
> (porta interna `OBSCURA_CHROME_INNER_PORT`, default 9224) e o **`cdp_proxy.py`**
> (proxy TCP unificado, substitui socat + health_server.py) expõe o CDP na
> rede: encaminha `/json/*` e `/devtools/*` reescrevendo Host→127.0.0.1
> (anti-DNS-rebinding) e responde **200 para qualquer caminho não-CDP** — a
> rede privada do Railway alcança o Chrome de verdade, e o healthcheck passa
> em qualquer porta.
